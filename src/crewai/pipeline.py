...
class CrewAIResumePipeline:
    ...
    def run(
        ...
        try:
            cache_text = jd[:2000] + "\n---EXP---\n" + raw_experience[:2000]
            set_cached("full_result", cache_text, json.dumps(result, ensure_ascii=False))
        except Exception:
    ...
    def _run_familiarity_check(self, jd: str) -> dict:
        ...
        cached = get_cached("familiarity", jd[:3000])
        if cached:
            try:
        ...
        # Cache the result
        set_cached("familiarity", jd[:3000], json.dumps(parsed, ensure_ascii=False))
    ...
    def _run_industry_decoding(self, jd: str) -> str:
        ...
        cached = get_cached("industry_decoding", jd[:3000])
        if cached:
        ...
        glossary = str(result)
        set_cached("industry_decoding", jd[:3000], glossary)
        if settings.verbose:
    ...
    def _run_jd_analysis(self, jd: str, industry_glossary: str = "") -> str:
        ...
        cached = get_cached("jd_analysis", jd[:3000])
        if cached:
        ...
        set_cached("jd_analysis", jd[:3000], jd_analysis)
    ...
    def _run_extraction(self, raw_experience: str, anchor_content: str) -> str:
        ...
        cached = get_cached("experience_diagnosis", cache_text)
        if cached:
        ...
        set_cached("experience_diagnosis", cache_text, phase1_summary)
